...
const App: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      console.error(e);
      setError(e.message === "API_KEY_INVALID" ? "配置错误：请检查 API Key。" : "网络似乎有些拥堵，请稍后再试。");
    } finally {
      setIsTyping(false);
    }
  };
    ...
    <div className="flex h-screen transition-colors duration-700 overflow-hidden" style={{ backgroundColor: currentTheme.bg }}>
      <aside className="w-80 border-r border-slate-200/20 hidden md:block" style={{ backgroundColor: currentTheme.sidebar }}>
        <MemorySidebar 
          ...
          onClear={() => { if(confirm("确定要开启新的篇章，清空所有心迹吗？")) { setMessages([]); setMemories([]); localStorage.clear(); }}} 
        />
      </aside>
      ...
      <main className="flex-1 flex flex-col relative max-w-4xl mx-auto w-full px-4 md:px-0">
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-6 md:px-10 pt-4 pb-44 space-y-2">
          ...
          {error && <div className="text-center p-3 text-[11px] text-red-400 bg-red-50/50 rounded-xl backdrop-blur-sm mx-auto max-w-xs">{error}</div>}
        </div>

        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10 bg-gradient-to-t from-transparent via-transparent to-transparent pointer-events-none">
